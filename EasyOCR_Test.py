...
# parser.add_argument('--pdf_folder', default='pdfFile/', type=str, help='folder path to PDF files')
...
#             image_path = os.path.join(output_folder, f"{os.path.splitext(pdf_file)[0]}_page_{i + 1}.jpg")
...
# text_result_folder = './textResult/'
#
# if not os.path.isdir(result_folder):
#     os.mkdir(result_folder)
# if not os.path.isdir(text_result_folder):
#     os.mkdir(text_result_folder)
#
#
...
#     args.cuda = False
#     # PDF 파일을 JPG 파일로 변환
#     convert_pdf_to_jpg(args.pdf_folder, args.test_folder, args.poppler_path)
#
#     # 변환된 이미지 파일 목록 업데이트
#     image_list = get_files_from_folder(args.test_folder)
#
#     # load net
...
#
#         # 텍스트 추출 및 저장
#         text_result_path = os.path.join(text_result_folder, filename + '.txt')
...
def preprocess_image(image):
    ...
    # 그레이스케일 이미지를 다시 3채널 이미지로 변환
    three_channel_image = cv2.cvtColor(denoised_image, cv2.COLOR_GRAY2BGR)
...
def test_net(net, image, text_threshold, link_threshold, low_text, cuda, poly, refine_net=None):
    ...
    # resize
    img_resized, target_ratio, size_heatmap = imgproc.resize_aspect_ratio(preprocessed_image, args.canvas_size,
                                                                          interpolation=cv2.INTER_LINEAR,
                                                                          mag_ratio=args.mag_ratio)
    ratio_h = ratio_w = 1 / target_ratio
